...
```
```{r}
nearest_to(races, races[["negro"]]) - nearest_to(races, races[["asiatic"]])
```
...
```
This is a very rough cut, but it does appear nearly all reference in the texts to the word "negro" is associated with discussion of eugenics or the observatios of an 18th century missionary.
...
3) create subsets along religious and nationality lines
4) create gender sets
5) create chronological sets (pre- and post- Civil War and early 20th century are some obvious breaks)
5) create public health sets (disease, disability, abortion, quarantine, vaccination) 
6) compare public health sets to race/gender/religion
